...
from app import (
    ...
    MINIMUM_DATA_POINTS_FOR_ALERT_PERCENTAGE,
...
class TestAlertsAPI(unittest.TestCase):
    ...
    def test_05_get_all_alerts_with_data(self):
        ...
        self.app.post(
            "/api/alerts",
            json={
                "alert_name": "Test CPU High List",
                "server_name": "local",
                "resource_type": "cpu",
                "threshold_percentage": 80.0,
                "time_window_minutes": 5,
                "emails": "list@example.com",
            },
        )
        response = self.app.get("/api/alerts")
        self.assertEqual(response.status_code, 200)
    ...
    def test_06_get_specific_alert_success(self):
        ...
        create_resp = self.app.post(
            "/api/alerts",
            json={
                "alert_name": "Specific Alert",
                "server_name": "server1",
                "resource_type": "ram",
                "threshold_percentage": 70.0,
                "time_window_minutes": 10,
                "emails": "spec@example.com",
            },
        )
    ...
    def test_08_update_alert_success(self):
        ...
        response = self.app.put(
            ...
            json={"alert_name": "Updated Name", "threshold_percentage": 65.0},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_12_enable_disable_alert(self):
        ...
        create_resp = self.app.post(
            "/api/alerts",
            json={
                "alert_name": "EnableDisable",
                "server_name": "local",
                "resource_type": "ram",
                "threshold_percentage": 77.0,
                "time_window_minutes": 7,
                "emails": "toggle@example.com",
                "is_enabled": True,
            },
        )
        alert_id = create_resp.get_json()["alert_id"]
...
class TestAlertEvaluation(unittest.TestCase):
    ...
    def _insert_stat_data(
        self, server_name, resource_type, value, timestamp_ago_minutes
    ):
        conn, cursor = get_db_connection()
    ...
    def test_alert_should_trigger(self, mock_send_email):
        ...
        alert_config = self._create_test_alert(
            time_window_minutes=2, threshold_percentage=50
        )
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT last_triggered_at FROM alerts WHERE id = ?", (alert_config["id"],)
        )
    ...
    @patch("app.send_alert_email")
    @patch(
        "app.collector_status_info",
        {"configured_server_names": ["serverA", "serverB", "local"]},
    )
    def test_alert_wildcard_server(self, mock_collector_info, mock_send_email):
        from app import evaluate_alerts

        self._create_test_alert(
            server_name="*", time_window_minutes=2, threshold_percentage=50
        )
...
class TestEmailSending(unittest.TestCase):
    ...
    def test_send_email_tls(self, mock_smtp, mock_smtp_ssl):
        ...
        self.assertListEqual(
            sorted(call_args[0][1]),
            sorted(["receiver1@example.com", "receiver2@example.com"]),
        )
        self.assertIn(
            "Subject: FalconEye Alert: TLS Test triggered on test_server",
            call_args[0][2],
        )
