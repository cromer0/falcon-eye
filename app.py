...
if __name__ == "__main__":
    ...
    if run_mode_for_thread != "development" and (
        ...
        logger.info(
            "FALCON_EYE_ENV is 'development', collector thread will be started by the main dev server process if app.run is called."
        )
    ...
    if run_mode == "development":
        ...
        if not app.debug or os.environ.get("WERKZEUG_RUN_MAIN") == "true":
            if (
                not "collector_thread" in locals() or not collector_thread.is_alive()
            ):  # Start if not already started by above block
                collector_thread = threading.Thread(
                    target=historical_data_collector,
                    name="HistoricalDataCollectorThread",
                    daemon=True,
                ...
                collector_thread.start()
                logger.info(
                    "Historical data collector thread started for development mode."
                )
        ...
        logger.info(
            f"Application ready to be served by a production WSGI server (e.g., Waitress). FALCON_EYE_ENV='{run_mode}'."
        )
        logger.info(
            "If running with Docker, Waitress is already configured in the Dockerfile."
        )
        logger.info(
            "If running 'python app.py' directly in a production-like environment, ensure Waitress or another WSGI server is used to serve 'app:app'."
        )
