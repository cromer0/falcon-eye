document.addEventListener("DOMContentLoaded", () => {
  ...
  const remoteServersTableBody = document.querySelector(
    "#remoteServersTable tbody",
  );
  const serverListLastUpdatedSpan = document.getElementById(
    "serverListLastUpdated",
  );

  const backToServerListButton = document.getElementById(
  ...
  const historicalDataMessageEl = document.getElementById(
    "historicalDataMessage",
  );
  const detailHistoricalChartContainerEl = document.getElementById(
    "detailHistoricalChartContainer",
  );
  const detailHistCpuChartEl = document
    .getElementById("detailHistoricalCpuChart")
    .getContext("2d");
  const detailHistRamChartEl = document
    .getElementById("detailHistoricalRamChart")
    .getContext("2d");
  const detailHistDiskChartEl = document
    .getElementById("detailHistoricalDiskChart")
    .getContext("2d");
  ...
  const cancelEditAlertButton = document.getElementById(
    "cancelEditAlertButton",
  );
  const alertsTableBody = document.getElementById("alertsTableBody");
  const userFeedbackDiv = document.getElementById("userFeedback");
  const userFeedbackMessageSpan = document.getElementById(
    "userFeedbackMessage",
  );
  ...
  let detailHistoricalCpuChart,
    detailHistoricalRamChart,
    detailHistoricalDiskChart;
  ...
  const SERVER_LIST_REFRESH_INTERVAL =
    window.APP_CONFIG && window.APP_CONFIG.serverListRefreshInterval
      ? window.APP_CONFIG.serverListRefreshInterval
      : 15000;
  // Use interval from Flask for detail view, with a fallback to 3000ms
  const DETAIL_VIEW_REFRESH_INTERVAL =
    window.APP_CONFIG && window.APP_CONFIG.detailViewRefreshInterval
      ? window.APP_CONFIG.detailViewRefreshInterval
      : 3000;
  ...
  function createDetailGauge(canvasId, value, color) {
    ...
    const trackColor = getCssVariable(
  ...
  function updateDetailGauge(chart, value) {
    // Generic updateGauge
    if (chart && chart.data) {
  ...
  function createDetailHistoricalChart(
    canvas,
    label,
    data,
    color,
    ...
    // Takes canvas context directly and allMetricsData
        ...
        scales: {
          x: {
            ...
            title: {
              display: true,
              text: "Time",
              color: themeOptions.scales.x.ticks.color,
          ...
          y: {
            ...
            title: {
              display: true,
              text: "Usage (%)",
              color: themeOptions.scales.y.ticks.color,
              font: { size: 12, weight: "bold" },
            },
          },
        },
            ...
            callbacks: {
              title: function (tooltipItems) {
                ...
                if (tooltipItems.length > 0) {
                  ...
                  if (
                    allMetricsData &&
                    allMetricsData.labels &&
                    allMetricsData.labels[dataIndex]
                  ) {
                    return new Date(
                      allMetricsData.labels[dataIndex],
                    ).toLocaleString();
              ...
              afterBody: function (tooltipItems) {
                ...
                if (tooltipItems.length > 0) {
                  ...
                  if (
                    ...
                    if (
                      allMetricsData.cpu &&
                      allMetricsData.cpu[dataIndex] !== undefined
                    ) {
                      lines.push(
                        `CPU: ${allMetricsData.cpu[dataIndex].toFixed(1)}%`,
                      );
                    }
                    if (
                      allMetricsData.ram &&
                      allMetricsData.ram[dataIndex] !== undefined
                    ) {
                      lines.push(
                        `RAM: ${allMetricsData.ram[dataIndex].toFixed(1)}%`,
                      );
                    }
                    if (
                      allMetricsData.disk &&
                      allMetricsData.disk[dataIndex] !== undefined
                    ) {
                      lines.push(
                        `Disk: ${allMetricsData.disk[dataIndex].toFixed(1)}%`,
                      );
                    }
                  }
                }
  ...
  function destroyDetailHistoricalCharts() {
    ...
    detailHistoricalCpuChart = null;
    detailHistoricalRamChart = null;
    detailHistoricalDiskChart = null;
    ...
    document
      .querySelectorAll("#detailHistoricalChartContainer .chart-container")
  ...
  function showView(viewId) {
    ...
    if (viewId === "serverListView" || viewId === "alertConfigView") {
      ...
      if (viewId !== "serverDetailView") {
        // only destroy if not going to detail view
    ...
    if (viewId === "serverDetailView") {
      if (detailViewInterval) clearInterval(detailViewInterval);
      detailViewInterval = setInterval(
        updateSelectedServerCurrentData,
        DETAIL_VIEW_REFRESH_INTERVAL,
      );
  ...
  function applyThemeToVisuals() {
    ...
    if (
      selectedServerData &&
      activeView &&
      activeView.id === "serverDetailView"
    ) {
      ...
      detailCpuGaugeChart = createDetailGauge(
        "detailCpuGauge",
        selectedServerData.cpu_percent || 0,
        getCssVariable("--gauge-cpu-color"),
      );
      detailRamGaugeChart = createDetailGauge(
        "detailRamGauge",
        selectedServerData.ram_percent || 0,
        getCssVariable("--gauge-ram-color"),
      );
      detailDiskGaugeChart = createDetailGauge(
        "detailDiskGauge",
        selectedServerData.disk_percent || 0,
        getCssVariable("--gauge-disk-color"),
      );

      // Re-create historical charts if they exist and data is available
      if (
        historicalChartsInitializedForDetail &&
        currentHistoricalAllMetrics.labels.length > 0
      ) {
        ...
        detailHistoricalCpuChart = createDetailHistoricalChart(
          detailHistCpuChartEl,
          "CPU Usage",
          {
            labels: currentHistoricalAllMetrics.labels,
            values: currentHistoricalAllMetrics.cpu,
          },
          getCssVariable("--chart-line-cpu"),
          currentHistoricalAllMetrics,
        );
        detailHistoricalRamChart = createDetailHistoricalChart(
          detailHistRamChartEl,
          "RAM Usage",
          {
            labels: currentHistoricalAllMetrics.labels,
            values: currentHistoricalAllMetrics.ram,
          },
          getCssVariable("--chart-line-ram"),
          currentHistoricalAllMetrics,
        );
        detailHistoricalDiskChart = createDetailHistoricalChart(
          detailHistDiskChartEl,
          "Disk Usage",
          {
            labels: currentHistoricalAllMetrics.labels,
            values: currentHistoricalAllMetrics.disk,
          },
          getCssVariable("--chart-line-disk"),
          currentHistoricalAllMetrics,
        );
      ...
      .then((servers) => {
        ...
        servers.forEach((server) => {
          ...
          if (server.status === "online") {
            ...
            diskCell.appendChild(
              createProgressBar(server.disk_percent, "Disk"),
            );
          } else {
            cpuCell.textContent = "N/A";
            ramCell.textContent = "N/A";
        ...
        if (serverListLastUpdatedSpan)
          serverListLastUpdatedSpan.textContent =
            new Date().toLocaleTimeString();
      })
      .catch((error) => {
        console.error("Error fetching server list:", error);
        if (remoteServersTableBody)
          remoteServersTableBody.innerHTML = `<tr><td colspan="6">Error loading server data.</td></tr>`;
      });
  }
  // ProgressBar for list view (same as detail view's createProgressBar)
  const createProgressBar = (percentage, type) => {
    /* ... same logic as createDetailProgressBar from previous answers ... */
  ...
  function handleServerSelect(serverHost) {
    ...
    document
      .querySelector('.detail-tab-button[data-tab="detail-current"]')
  ...
  function updateSelectedServerCurrentData() {
    if (!selectedServerData || !selectedServerData.host) {
      console.error(
        "No server selected or host missing, cannot update detail view.",
      );
    ...
    fetch(`/api/remote_servers_stats?host=${selectedServerData.host}`)
      ...
      .then((freshDataResponse) => {
        ...
        if (Array.isArray(freshDataResponse) && freshDataResponse.length > 0) {
          ...
          Array.isArray(freshDataResponse) &&
          freshDataResponse.length === 0
        ) {
          console.error(
            "Received empty array for host (server might have been removed or host is incorrect):",
            selectedServerData.host,
          );
          ...
          if (
            typeof freshDataResponse === "object" &&
            freshDataResponse !== null &&
            !Array.isArray(freshDataResponse) &&
            freshDataResponse.host === selectedServerData.host
          ) {
            freshData = freshDataResponse;
          } else {
            console.error(
              "Received unexpected data format for host:",
              selectedServerData.host,
              freshDataResponse,
            );
            if (detailLastUpdatedEl) {
      ...
      .catch((error) => {
        console.error(
          "Error fetching details for server:",
          selectedServerData.host,
          error,
        );
        if (detailLastUpdatedEl) {
  ...
  function handleHistoricalTabForSelectedServer() {
    ...
    if (selectedServerData) {
      ...
      fetch(url)
        ...
        .then((data) => {
          if (
            ...
            document
              .querySelectorAll(
                "#detailHistoricalChartContainer .chart-container",
              )
            ...
            historicalDataMessageEl.textContent =
              "No historical data available for this server.";
          }
        })
        .catch((error) => {
          console.error(
            `Error fetching historical data for ${serverName}:`,
            error,
          );
          historicalDataMessageEl.textContent =
      ...
      historicalDataMessageEl.textContent =
        "Historical data is only available for the local server instance defined in the configuration."; // This message might need adjustment if historical data can be fetched for remotes.
      historicalDataMessageEl.style.display = "block";
    }
  }
  ...
  backToServerListButton.addEventListener("click", () =>
    ...
    fetch("/api/collector_status")
      ...
      .then((data) => {
        ...
        serverNameSelect.innerHTML =
          '<option value="" disabled>Select Server</option>'; // Clear existing
        serverNameSelect.add(new Option("All Servers (*)", "*"));
        serverNameSelect.add(new Option("Local Server (local)", "local"));

        if (
          data.configured_server_names &&
          Array.isArray(data.configured_server_names)
        ) {
        ...
        if (
          Array.from(serverNameSelect.options).some(
          ...
          const firstEnabledOption = Array.from(serverNameSelect.options).find(
            (opt) => !opt.disabled,
          );
          if (firstEnabledOption)
            serverNameSelect.value = firstEnabledOption.value;
      ...
      .catch((error) => {
        ...
        serverNameSelect.innerHTML =
          '<option value="" disabled selected>Error loading servers</option>';
      });
  }
  ...
  function fetchAndDisplayAlerts() {
    fetch("/api/alerts")
      ...
      .then((alerts) => {
        ...
        if (alerts.length === 0) {
          alertsTableBody.innerHTML =
            '<tr><td colspan="10" style="text-align:center;">No alerts configured yet.</td></tr>';
          return;
        }
        alerts.forEach((alert) => {
          ...
          statusBadge.className = alert.is_enabled
            ? "status-enabled"
            : "status-disabled";
          statusCell.appendChild(statusBadge);

          row.insertCell().textContent = alert.last_triggered_at
          ...
          toggleEnableButton.textContent = alert.is_enabled
            ? "Disable"
            : "Enable";
      ...
      .catch((error) => {
        ...
        alertsTableBody.innerHTML =
          '<tr><td colspan="10" style="text-align:center;">Error loading alerts.</td></tr>';
  ...
  alertConfigForm.addEventListener("submit", function (event) {
    ...
    if (
      ...
      isNaN(formData.threshold_percentage) ||
      isNaN(formData.time_window_minutes) ||
      !formData.emails
    ) {
      showFeedback(
    ...
    if (
      formData.threshold_percentage < 0 ||
      formData.threshold_percentage > 100
    ) {
    ...
    fetch(url, {
      ...
      .then((res) => {
        if (res.ok) {
          showFeedback(
            `Alert ${alertId ? "updated" : "created"} successfully.`,
            "success",
          );
          resetAlertForm();
          fetchAndDisplayAlerts();
          ...
          throw new Error(
            res.body.error ||
              `Failed to ${alertId ? "update" : "create"} alert. Status: ${res.status}`,
          );
        }
      })
      .catch((error) => {
        console.error(
          `Error ${alertId ? "updating" : "creating"} alert:`,
          error,
        );
  ...
  function handleEditAlert(event) {
    ...
    fetch(`/api/alerts/${alertId}`)
      ...
      .then((alert) => {
        ...
        window.scrollTo({
          top: alertConfigForm.offsetTop - 20,
          behavior: "smooth",
        }); // Scroll to form
      })
      .catch((error) => {
      ...
      fetch(`/api/alerts/${alertId}`, { method: "DELETE" })
        ...
        .then((res) => {
          if (res.ok) {
            ...
            throw new Error(
              res.body.error || `Failed to delete alert. Status: ${res.status}`,
            );
          }
        })
        .catch((error) => {
    ...
    fetch(`/api/alerts/${alertId}/${action}`, { method: "POST" })
      ...
      .then((res) => {
        if (res.ok) {
          ...
          throw new Error(
            res.body.error ||
              `Failed to ${action} alert. Status: ${res.status}`,
          );
        }
      })
      .catch((error) => {
  ...
  navServerList.addEventListener("click", (e) => {
    e.preventDefault();
    showView("serverListView");
  });
  navAlertConfig.addEventListener("click", (e) => {
    e.preventDefault();
    showView("alertConfigView");
  });
